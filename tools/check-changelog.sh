...
if [ -z "$BASE_REF" ]; then
  ...
  EXEMPT=$(echo "$STAGED_FILES" | grep -vE '^(CHANGELOG\.md|changelog\.d/|docs/|\.github/|\.githooks/|tools/check-changelog\.sh|.*\.md|.*\.nix|\.envrc|\.gitignore|\.gitattributes|bunfig\.toml|package\.json|bun\.lock|\.config/|supply-chain/|fuzz/|Cargo\.lock)$' || true)
  if [ -z "$EXEMPT" ]; then
    exit 0
  fi
